...
* `client.filters.colorCacheSize` -- cache size for each `palette` (*default:* `1024`).<br>
 It is not recommended changing it to a much bigger number, as it affects memory consumption. Changing it to values less than `256` is also not recommended.<br>
...
* `media.backupPath` -- path to back up media items (*default:* `null`)
